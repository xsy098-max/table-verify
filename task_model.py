...
此积木块已合并到「拆分」中。请使用新的「拆分」积木块，选择"单次拆分"模式即可。
""",
    },
    'flat_split': {
        'title': '✂ 拆分',
...
支持两种模式：
  • 单次拆分：用一个分隔符拆一次，如 "A|B|C" → ["A", "B", "C"]
  • 多级拆分：用多个分隔符逐级拆，如 "5_7|8_9" → [5, 7, 8, 9]
...
■ 输出是什么
  ...
  不填标签变量时（默认）：
    单次拆分 → 一个列表，如 ["苹果", "香蕉", "橘子"]
    多级拆分 → 展平成一个列表，如 [5, 7, 34, 35, 50, 57]
...
■ 小贴士
  ...
  • 单次拆分适合简单场景（一个分隔符就够）
  • 多级拆分的分隔符是按顺序逐级拆分的：先按第1个拆，再按第2个拆
  • 标签变量在两种模式下都可用，填了就输出字典
...
BLOCK_PARAMS = {
    ...
    'flat_split': [
        {'key': 'input_var', 'label': '输入变量', 'type': 'variable'},
        {'key': 'split_mode', 'label': '拆分模式', 'type': 'choice', 'choices': ['single', 'multi'], 'choice_labels': ['单次拆分', '多级拆分']},
        {'key': 'delimiters', 'label': '分隔符', 'type': 'text'},
